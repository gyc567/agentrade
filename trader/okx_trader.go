...
// placeAlgoOrder 下单条件单/策略单统一方法 (止损/止盈)
func (t *OKXTrader) placeAlgoOrder(order map[string]string) (map[string]interface{}, error) {
        // OKX API: POST /api/v5/trade/order-algo (策略订单专用端点)
        endpoint := "/api/v5/trade/order-algo"

        log.Printf("📤 OKX策略单请求: %+v", order)

        resp, err := t.makeRequest("POST", endpoint, order)
        if err != nil {
                return nil, fmt.Errorf("OKX策略单失败: %w", err)
        }
        ...
        if data, ok := resp["data"].([]interface{}); ok && len(data) > 0 {
                if orderResp, ok := data[0].(map[string]interface{}); ok {
                        ...
                        sMsg, _ := orderResp["sMsg"].(string)
                        if sCode != "" && sCode != "0" {
                                log.Printf("❌ OKX策略单详细错误: sCode=%s, sMsg=%s", sCode, sMsg)
                                return nil, fmt.Errorf("OKX策略单失败 [%s]: %s", sCode, sMsg)
                        }
                        // 获取策略单ID
                        if algoId, ok := orderResp["algoId"].(string); ok && algoId != "" {
                                log.Printf("✅ OKX策略单成功: algoId=%s, side=%s, symbol=%s, quantity=%s",
                                        algoId, order["side"], order["instId"], order["sz"])
                        }
                }
        }
...
func (t *OKXTrader) SetStopLoss(symbol string, positionSide string, quantity, stopPrice float64) error {
        ...
        // 使用 order-algo 端点的条件单参数
        // 止损使用 slTriggerPx 和 slOrdPx
        order := map[string]string{
                "instId":      okxSymbol,
                "tdMode":      "cross",
                "side":        side,
                ...
                "sz":          strconv.FormatFloat(quantity, 'f', -1, 64),
                "slTriggerPx": strconv.FormatFloat(stopPrice, 'f', -1, 64), // 止损触发价格
                "slOrdPx":     "-1",                                        // 市价触发
        }

        _, err := t.placeAlgoOrder(order)
        if err != nil {
...
func (t *OKXTrader) SetTakeProfit(symbol string, positionSide string, quantity, takeProfitPrice float64) error {
        ...
        order := map[string]string{
                ...
                "ordType":     "conditional",
                "sz":          strconv.FormatFloat(quantity, 'f', -1, 64),
                "tpTriggerPx": strconv.FormatFloat(takeProfitPrice, 'f', -1, 64), // 止盈触发价格
                "tpOrdPx":     "-1", // 市价触发
        }
