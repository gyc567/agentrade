...
// CheckTransactionExists 检查交易流水是否已存在（幂等性检查）
...
// ReserveCreditForTrade 预留积分用于交易（开始事务）
// 返回事务对象和扣减前余额，调用者需要调用 ConfirmCreditConsumption 或 ReleaseCreditReservation
func (d *Database) ReserveCreditForTrade(userID string, amount int) (*sql.Tx, int, error) {
        ...
        if err != nil {
                tx.Rollback()
                if err == sql.ErrNoRows {
                        return nil, 0, fmt.Errorf("用户积分账户不存在: %s", userID)
                }
                return nil, 0, fmt.Errorf("获取用户积分失败: %w", err)
        }

        // 检查积分是否足够
        if availableCredits < amount {
                tx.Rollback()
                return nil, 0, fmt.Errorf("积分不足: 当前 %d, 需要 %d", availableCredits, amount)
        }
        ...
        if err != nil {
                tx.Rollback()
                return nil, 0, fmt.Errorf("预扣减积分失败: %w", err)
        }

        log.Printf("🔒 用户 %s 积分已预留: %d (余额: %d -> %d)", userID, amount, availableCredits, availableCredits-amount)
        return tx, availableCredits, nil
}

// ConfirmCreditConsumption 确认积分消费（提交事务）
// 记录交易流水并提交事务
func (d *Database) ConfirmCreditConsumption(tx *sql.Tx, userID, tradeID, description string, amount, balanceBefore int) error {
        ...
        if err != nil {
                tx.Rollback()
                return fmt.Errorf("更新已使用积分失败: %w", err)
        }
        ...
        log.Printf("✅ 用户 %s 积分消费确认: %d (tradeID: %s)", userID, amount, tradeID)
        return nil
}

// ReleaseCreditReservation 释放积分预留（回滚事务）
// 交易失败时调用，回滚之前预扣减的积分
